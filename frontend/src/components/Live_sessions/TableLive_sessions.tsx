...
import { loadColumns } from './configureLive_sessionsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListLive_sessions from './ListLive_sessions';

const perPage = 10;

const TableSampleLive_sessions = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: live_sessionsNotify,
    refetch,
  } = useAppSelector((state) => state.live_sessions);
  ...
  useEffect(() => {
    if (live_sessionsNotify.showNotification) {
      notify(
        live_sessionsNotify.typeNotification,
        live_sessionsNotify.textNotification,
      );
    }
  }, [live_sessionsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/live_sessions/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/live_sessions/live_sessions-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {live_sessions && Array.isArray(live_sessions) && !showGrid && (
        <ListLive_sessions
...
export default TableSampleLive_sessions;
